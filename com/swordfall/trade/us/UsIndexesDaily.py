...
class UsIndexesDaily:
    ...
    def parsing_us_plate_substock(self, salt, id):
        headers = {
            "User-Agent": 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/83.0.4103.116 Safari/537.36'}
    ...
    def get_us_plate_substock(self):
        ...
        print("------ 采矿 -------")
        (mining_count, mining_data) = self.parsing_us_plate_substock('0BGdPLKaSOnAYUKL', '2')
        self.update_us_index_substock_list_exist(31, mining_data, 'us_plate_mining_substock_list',  mining_count)

        print("------ 钢铁 -------")
        (steel_count, steel_data) = self.parsing_us_plate_substock('QRhIHyKEpcO0MInd', '46')
        self.update_us_index_substock_list_exist(32, steel_data, 'us_plate_steel_substock_list', steel_count)

        print("------ 化学制品 -------")
        (chemicals_count, chemicals_data) = self.parsing_us_plate_substock('8Wp$XMIlfTVJcCHy', '88')
        ...
        print("------ 计算机 -------")
        (compute_count, compute_data) = self.parsing_us_plate_substock('ilPtHiClt_p8HArD', '5')
        self.update_us_index_substock_list_exist(35, compute_data, 'us_plate_compute_substock_list', compute_count)

        print("------ 半导体 -------")
        (semiconductor_count, semiconductor_data) = self.parsing_us_plate_substock('uAZnIudtFy7PjpOt', '12')
        self.update_us_index_substock_list_exist(36, semiconductor_data, 'us_plate_semiconductor_substock_list', semiconductor_count)

        print("------ 软件 -------")
        (software_count, software_data) = self.parsing_us_plate_substock('TvDHcqIaIf98HArD', '14')
        ...
        print("------ 计算机硬件/企业设备 -------")
        (hardware_count, hardware_data) = self.parsing_us_plate_substock('8Wp$XMoQpRW4cCHy', '681')
        self.update_us_index_substock_list_exist(39, hardware_data, 'us_plate_hardware_substock_list', hardware_count)

        print("------ 电力组件与设备 -------")
        (power_components_count, power_components_data) = self.parsing_us_plate_substock('tnuEDFReoimdDZGk', '7')
        ...
        print("------ 运输 -------")
        (transport_count, transport_data) = self.parsing_us_plate_substock('B4FC3grlt_p8HArD', '34')
        self.update_us_index_substock_list_exist(41, transport_data, 'us_plate_transport_substock_list', transport_count)

        print("------ 电子 -------")
        (electronic_count, electronic_data) = self.parsing_us_plate_substock('8Wp$XMIifTVJcCHy', '49')
        self.update_us_index_substock_list_exist(42, electronic_data, 'us_plate_electronic_substock_list', electronic_count)

        print("------ 多元化制造商 -------")
        (diversified_manufacturers_count, diversified_manufacturers_data) = self.parsing_us_plate_substock('CvDHcqNHIf98HArD', '58')
        ...
        print("------ 航空航天/国防 -------")
        (aerospace_count, aerospace_data) = self.parsing_us_plate_substock('nHH$Ii0iEx6NC5Ap', '60')
        self.update_us_index_substock_list_exist(44, aerospace_data, 'us_plate_aerospace_substock_list', aerospace_count)

        print("------ 包装容器 -------")
        (packaging_count, packaging_data) = self.parsing_us_plate_substock('az83lL34XmFyuF_K', '65')
        ...
        print("------ 制药 -------")
        (pharmaceutical_count, pharmaceutical_data) = self.parsing_us_plate_substock('FF3AiCNuoimdDZGk', '10')
        ...
        print("------ 商业服务 -------")
        (business_service_count, business_service_data) = self.parsing_us_plate_substock('JyRdhgLht_p8HArD', '16')
        ...
        print("------ 食品 -------")
        (food_count, food_data) = self.parsing_us_plate_substock('CvDHcqNHIf98HArD', '18')
        self.update_us_index_substock_list_exist(57, food_data, 'us_plate_food_substock_list', food_count)

        print("------ 生物技术 -------")
        (biotechnology_count, biotechnology_data) = self.parsing_us_plate_substock('wdv5LWh7fBt9Rqml', '39')
        self.update_us_index_substock_list_exist(58, biotechnology_data, 'us_plate_biotechnology_substock_list', biotechnology_count)

        print("------ 化妆品/护理 -------")
        (cosmetics_nursing_count, cosmetics_nursing_data) = self.parsing_us_plate_substock('TvDHcqIaIf98HArD', '54')
        ...
        print("------ 医疗保健产品 -------")
        (healthcare_products_count, healthcare_products_data) = self.parsing_us_plate_substock('cgntpUixS198GJY3', '63')
        ...
        print("------ 饮料 -------")
        (drinks_count, drinks_data) = self.parsing_us_plate_substock('B4FC3grlt_p8HArD', '74')
        self.update_us_index_substock_list_exist(61, drinks_data, 'us_plate_drinks_substock_list', drinks_count)

        print("------ 健康护理服务 -------")
        (health_care_services_count, health_care_services_data) = self.parsing_us_plate_substock('TvDHcqIBIf98HArD', '95')
        ...
        print("------ 农业 -------")
        (agriculture_count, agriculture_data) = self.parsing_us_plate_substock('x_p$WQthguXxK5ae', '159')
        ...
        print("------ 电力 -------")
        (electricity_count, electricity_data) = self.parsing_us_plate_substock('nHH$Ii09Ex6NC5Ap', '21')
        ...
        print("------ 多元化金融服务 -------")
        (diversified_financial_services_count, diversified_financial_services_data) = self.parsing_us_plate_substock('JyRdhgLht_p8HArD', '56')
        ...
        print("------ 银行 -------")
        (bank_count, bank_data) = self.parsing_us_plate_substock('nHH$Ii09Ex6NC5Ap', '61')
        ...
        print("------ 电信 -------")
        (telecommunications_count, telecommunications_data) = self.parsing_us_plate_substock('cgntpUyzS198GJY3', '30')
        ...
        print("------ 航空公司 -------")
        (airline_count, airline_data) = self.parsing_us_plate_substock('cgntpUSoWa58GJY3', '103')
        self.update_us_index_substock_list_exist(84, airline_data, 'us_plate_airline_substock_list', airline_count)

        print("------ 房屋建造商 -------")
        (house_builder_count, house_builder_data) = self.parsing_us_plate_substock('cgntpUSv9JTQGJY3', '110')
        self.update_us_index_substock_list_exist(85, house_builder_data, 'us_plate_house_builder_substock_list', house_builder_count)

        print("------ 家具家饰 -------")
        (furniture_furnishings_count, furniture_furnishings_data) = self.parsing_us_plate_substock('ilPtHiGw4BCfHArD', '139')
        ...
        print("------ 服饰 -------")
        (clothes_count, clothes_data) = self.parsing_us_plate_substock('x_p$WQtmbE9iK5ae', '148')
        self.update_us_index_substock_list_exist(87, clothes_data, 'us_plate_clothes_substock_list', clothes_count)

        print("------ 汽车制造 -------")
        (car_manufacturer_count, car_manufacturer_data) = self.parsing_us_plate_substock('6MizKiL4$yTAC5Ap', '155')
        ...
        print("------ 住宿 -------")
        (stay_count, stay_data) = self.parsing_us_plate_substock('K4FC3gL_V9t8HArD', '176')
        self.update_us_index_substock_list_exist(89, stay_data, 'us_plate_stay_substock_list', stay_count)

        print("------ 休闲 -------")
        (casual_count, casual_data) = self.parsing_us_plate_substock('O0j3lgNwDBYSFo4p', '222')
        self.update_us_index_substock_list_exist(90, casual_data, 'us_plate_casual_substock_list', casual_count)

        time.sleep(2)

        print("------ 销售/批发 -------")
        (sales_wholesale_count, sales_wholesale_data) = self.parsing_us_plate_substock('O0j3lgC$FqXbFo4p', '256')
        ...
        print("------ 纺织品 -------")
        (textile_count, textile_data) = self.parsing_us_plate_substock('yjD5LWn3nCEwRqml', '319')
        ...
        print("------ 油气服务 -------")
        (oil_and_gas_service_count, oil_and_gas_service_data) = self.parsing_us_plate_substock('SGJ4pB4buMz9JptU', '68')
        ...
        print("------ 油气 -------")
        (oil_and_gas_count, oil_and_gas_data) = self.parsing_us_plate_substock('az83lL3lXmFyuF_K', '76')
        self.update_us_index_substock_list_exist(98, oil_and_gas_data, 'us_plate_oil_and_gas_substock_list', oil_and_gas_count)

        print("------ 能源-替代能源 -------")
        (alternative_energy_count, alternative_energy_data) = self.parsing_us_plate_substock('tw7ZFdi4iNBJFo4p', '109')
        ...
        print("------ 管道 -------")
        (pipeline_count, pipeline_data) = self.parsing_us_plate_substock('cgntpUSv9JTQGJY3', '112')
        self.update_us_index_substock_list_exist(100, pipeline_data, 'us_plate_pipeline_substock_list', pipeline_count)

        print("------ 煤炭 -------")
        (coal_count, coal_data) = self.parsing_us_plate_substock('tw7ZFdS$PtW_Fo4p', '210')
